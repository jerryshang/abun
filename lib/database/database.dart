...
class AppDatabase extends _$AppDatabase {
  ...
  Future<List<Session>> getAllSessions() async {
    final results = await select(sessions).get();
  ...
  Stream<List<Session>> watchAllSessions() {
    return select(sessions).watch().map((rows) => rows.map((row) => Session(
      id: row.id,
      taskId: row.taskId,
      title: row.title,
      startTime: row.startTime,
  ...
  Future<List<Session>> getSessionsByTask(String taskId) async {
    final results = await (select(sessions)..where((s) => s.taskId.equals(taskId))).get();
    return results.map((row) => Session(
      id: row.id,
      taskId: row.taskId,
      title: row.title,
  ...
  Future<String> createSession(SessionsCompanion session) {
    // Generate a UUID if not provided
    final sessionWithId = session.id.present ? session : session.copyWith(
      id: Value(const Uuid().v4()),
    );

    return into(sessions).insert(sessionWithId).then((_) => sessionWithId.id.value);
  }
