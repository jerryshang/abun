...
class FinanceCategoryRepository(private val database: AppDatabase) {
   ...
   */
  fun getAllCategoriesFlow(): Flow<List<FinanceCategory>> = queries.getAllFinanceCategories().asFlow().mapToList(Dispatchers.IO).map { list ->
   ...
   * Get all categories
   */
  suspend fun getAllCategories(): List<FinanceCategory> = withContext(Dispatchers.IO) {
   ...
   * Get category by ID
   */
  suspend fun getCategoryById(id: Long): FinanceCategory? = withContext(Dispatchers.IO) {
   ...
   */
  suspend fun getCategoriesByType(type: CategoryType): List<FinanceCategory> = withContext(Dispatchers.IO) {
   ...
   */
  suspend fun getTopLevelCategories(): List<FinanceCategory> = withContext(Dispatchers.IO) {
   ...
   */
  suspend fun getSubCategories(parentId: Long): List<FinanceCategory> = withContext(Dispatchers.IO) {
   ...
   */
  suspend fun getCategoriesWithSubcategories(): List<CategoryWithSubcategories> = withContext(Dispatchers.IO) {
   ...
   */
  suspend fun createCategory(input: CreateCategoryInput): Long = withContext(Dispatchers.IO) {
   ...
   * Delete a category
   */
  suspend fun deleteCategory(id: Long): Unit = withContext(Dispatchers.IO) {
   ...
   */
  private fun DbFinanceCategory.toDomain() = FinanceCategory(
    id = id,
    name = name,
    parentId = parent_id,
