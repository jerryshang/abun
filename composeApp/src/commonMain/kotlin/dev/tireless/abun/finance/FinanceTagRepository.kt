...
class FinanceTagRepository(private val database: AppDatabase) {
   ...
   */
  fun getAllTagsFlow(): Flow<List<FinanceTag>> = queries.getAllFinanceTags().asFlow().mapToList(Dispatchers.IO).map { list ->
   ...
   * Get all tags
   */
  suspend fun getAllTags(): List<FinanceTag> = withContext(Dispatchers.IO) {
   ...
   * Get tag by ID
   */
  suspend fun getTagById(id: Long): FinanceTag? = withContext(Dispatchers.IO) {
   ...
   * Get tag by name
   */
  suspend fun getTagByName(name: String): FinanceTag? = withContext(Dispatchers.IO) {
   ...
   * Create a new tag
   */
  suspend fun createTag(input: CreateTagInput): Long = withContext(Dispatchers.IO) {
   ...
   * Delete a tag
   */
  suspend fun deleteTag(id: Long): Unit = withContext(Dispatchers.IO) {
   ...
   */
  suspend fun getOrCreateTag(name: String, colorHex: String? = null): FinanceTag = withContext(Dispatchers.IO) {
   ...
   */
  private fun DbFinanceTag.toDomain() = FinanceTag(
    id = id,
    name = name,
    colorHex = color_hex,
